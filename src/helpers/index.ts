...
export function isNoteHigher(
	...
	if (
		...
		return (
			indexOfAlteration(reference) < indexOfAlteration(expectedHigherNote)
		);
	}
...
export function isNoteLower(reference: Note, expectedLowerNote: Note): boolean {
	...
	if (
		...
		return (
			indexOfAlteration(reference) > indexOfAlteration(expectedLowerNote)
		);
	}
...
export function getNextNoteOnScale(scale: IScaleDegree[], note: Note) {
	const index = scale
		.map((scaleDegree) => scaleDegree.name)
		.indexOf(note.name);
