import * as isUnison from "@/modules/helpers/isUnison";
...
describe("isUnison", () => {
    ...
    it("is false for two different notes", () => {
        ...
        expect(isUnison.isUnison(testsHelpers.G2, testsHelpers.Aflat2)).toBe(
            false
        );
        expect(isUnison.isUnison(testsHelpers.G2, testsHelpers.Bflat2)).toBe(
            false
        );
    });
    ...
    it("is true for enharmonies between F and E", () => {
        expect(isUnison.isUnison(testsHelpers.Fflat4, testsHelpers.E4)).toBe(
            true
        );
        expect(isUnison.isUnison(testsHelpers.E4, testsHelpers.Fflat4)).toBe(
            true
        );
    });
    ...
    test("special case", () => {
        ...
        expect(isUnison.isUnison(testsHelpers.Aflat1, testsHelpers.A1)).toBe(
            false
        );
        expect(isUnison.isUnison(testsHelpers.Asharp1, testsHelpers.A1)).toBe(
            false
        );
        expect(isUnison.isUnison(testsHelpers.A1, testsHelpers.Aflat1)).toBe(
            false
        );
        expect(
            isUnison.isUnison(testsHelpers.Aflat1, testsHelpers.Aflat1)
        ).toBe(true);
    });
    ...
    describe("in strict mode", () => {
        it("is false for enharmonics", () => {
            ...
            expect(
                isUnison.isUnison(testsHelpers.Cb4, testsHelpers.B3, true)
            ).toBe(false);
            expect(
                isUnison.isUnison(testsHelpers.B3, testsHelpers.Cb4, true)
            ).toBe(false);
        });
    });
});
