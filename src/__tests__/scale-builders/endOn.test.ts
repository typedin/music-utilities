import { getFirstPossibleNoteInScale } from "../../helpers/index.js";
import { beginOn } from "../../scale-builders/beginOn.js";
import { endOn } from "../../scale-builders/endOn.js";
...
describe("endOn", () => {
  ...
  it.skip("can limit when the beginning is not the root", () => {
    const scale = beginOn(Scales.C_MAJOR_SCALE, testsHelpers.E1);
  ...
  it("does not go over the limit with complex chaining", () => {
    ...
    const result = endOn(beginOn(scale, possibleLowerLimit), upperLimit!);
