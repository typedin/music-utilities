import { DiatonicNoteEnum } from "@/modules/constants";
...
function areNotesTheSame(firstNote: Note, secondNote: Note): boolean {
    return (
        ...
        firstNote.name == secondNote.name
    );
}
...
export function isUnison(
    ...
    if (isNoteB(firstNote) && isComparingCandB(firstNote, secondNote)) {
        return (
            getPreviousAlteration(firstNote.alteration) == secondNote.alteration
        );
    }
    ...
    if (areNotesSpecialSecondsDown(firstNote, secondNote)) {
        return (
            getPreviousAlteration(secondNote.alteration) == firstNote.alteration
        );
    }
